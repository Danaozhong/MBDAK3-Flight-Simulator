...
type
  ...
  public
    ...
    function Create_(pd3dDevice: IDirect3DDevice8; const strFilename: PChar; const txtFolderName: String): HRESULT; overload;
...
//-----------------------------------------------------------------------------
  ...
  public
    ...
    function Render(pd3dDevice: IDirect3DDevice8): HRESULT;
...
// Desc:
//-----------------------------------------------------------------------------
function CD3DMesh.Create_(pd3dDevice: IDirect3DDevice8; const strFilename: PChar; const txtFolderName: String): HRESULT;
var
  strPath: array[0..MAX_PATH-1] of Char;
  strPathANSI: array[0..MAX_PATH-1] of Char;
  pMtrlBuffer: ID3DXBuffer;
  TempBuffer: ID3DXBuffer;
  d3dxMtrls: PAD3DXMATERIAL;
  i:dword;
  strTexture: array[0..MAX_PATH-1] of Char;
  strTextureANSI: array[0..MAX_PATH-1] of Char;
begin
  pMtrlBuffer:= nil;
  ...
  Result:= D3DXLoadMeshFromX(strPathANSI, D3DXMESH_SYSTEMMEM,
                             pd3dDevice, TempBuffer, pMtrlBuffer,
  ...
  begin
    ...
    begin
      ...
      begin
        DXUtil_ConvertGenericStringToAnsi(strTextureANSI, d3dxMtrls^[i].pTextureFilename, -1);
        DXUtil_FindMediaFile(strTexture, strTextureANSI);

        if (FAILED(D3DXCreateTextureFromFile(pd3dDevice, PChar(txtFolderName + strTexture),
...
begin
  ...
  Result:= D3DXLoadMeshFromXof(pFileData, D3DXMESH_SYSTEMMEM, pd3dDevice,
                               nil, @pMtrlBuffer, @m_dwNumMaterials,
                               m_pSysMemMesh);
  if FAILED(Result) then Exit;
...
// Desc:
//-----------------------------------------------------------------------------
function CD3DFile.Render(pd3dDevice: IDirect3DDevice8): HRESULT;
var
  matSavedWorld, matWorld: TD3DXMatrix;
begin
